...
class State:
    ...
    def apply(self, m):
        ...
        if t == "welcome":
            ...
            if "timer_duration" in m: self.timer_duration = int(m.get("timer_duration", 0))
            if "last_move_ts"  in m: self.last_move_ts  = float(m.get("last_move_ts", 0))
            ...
            self.room = m.get("room", self.room if self.room!="?" else "default")
            self.in_queue = False; pygame.time.set_timer(QUEUE_TICK, 0)
            self.status = f"Bạn cầm {self.me}. " + ("Đi trước." if your_turn else "Đợi đối thủ.")
        elif t == "state":
            grid = m.get("grid")
            if grid: self.grid = [row[:] for row in grid]
            self.turn = m.get("turn", self.turn)
            self.winner = m.get("winner"); self.draw = bool(m.get("draw", False))
            # timer từ server mới
            self.timer_duration = int(m.get("timer_duration", self.timer_duration or 0))
            self.last_move_ts   = float(m.get("last_move_ts", self.last_move_ts or 0))
            if not (self.winner or self.draw):
                self.status = "Đến lượt bạn" if self.me == self.turn else "Chờ đối thủ..."
                last = m.get("last") or {}
                if last and last.get("player") and last.get("player") != self.me:
                    lx, ly = last.get("x"), last.get("y")
                    self.status = f"Đối thủ đi ({lx},{ly}). " + self.status
        elif t == "chat":
            sender = m.get("from") or ""
            msg = m.get("message") or ""
            self.status = f"[Chat] {sender+': ' if sender else ''}{msg}"
        elif t == "undo_offer":
            self.pending_undo_id = m.get("req_id")
            steps = m.get("steps", 1)
            self.status = f"Đối thủ xin Undo ({steps} nước). Nhấn Y để đồng ý, N để từ chối."
        elif t == "undo_result":
            if m.get("ok"):
                s = m.get("state", {})
                # đồng bộ lại bàn nếu server gửi đầy đủ state
                self.grid   = s.get("board", self.grid)
                self.turn   = s.get("turn",  self.turn)
                self.winner = s.get("winner")
                self.draw   = s.get("draw", False)
                self.win_line = s.get("win_line", self.win_line)
                self.status = "Đã Undo."
            else:
                self.status = "Đối thủ từ chối Undo."
            self.pending_undo_id = None
st = State()
...
def draw_status(st, net):
    ...
    if st.time_remaining > 0:
        line += f"  •  ⏱ {int(st.time_remaining)}s"
...
def main():
    running=True
    while running:
        # tính timer mỗi frame từ timestamp server
        if st.timer_duration > 0 and st.last_move_ts and not (st.winner or st.draw):
            st.time_remaining = max(0, st.timer_duration - (time.time() - st.last_move_ts))
        else:
            st.time_remaining = 0
        for e in pygame.event.get():
            if e.type==pygame.QUIT:
                ...
                if e.key==pygame.K_ESCAPE:
                    ...
                    if not net.connected:
                        ...
                        if ok:
                            ...
                            st.in_queue=True
                            net.send({"type":"join","name":"Player","room":"default"})
                ...
                elif e.key==pygame.K_y and net.connected and st.pending_undo_id:
                    net.send({"type":"undo_accept","req_id": st.pending_undo_id})

                elif e.key==pygame.K_n and net.connected and st.pending_undo_id:
                    net.send({"type":"undo_reject","req_id": st.pending_undo_id})
                    st.pending_undo_id = None

                elif e.key==pygame.K_n and net.connected:
                    net.send({"type":"reset"})

                elif e.key==pygame.K_u and net.connected:
                    net.send({"type":"undo_request","steps":1})
                    st.status = "Đã gửi yêu cầu Undo..."
